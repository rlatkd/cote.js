...
| 서비스 | 기술 | 책임 |
|---|---|---|
| Frontend | Next.js | 문제 조회·풀이·제출 UI, 채점 결과·랭킹 |
| Backend API | Kotlin + Spring Boot | 회원·인증, 문제·제출·랭킹 등 비즈니스 로직 |
| AI 문제 생성 | Python + FastAPI + LLM API/LangChain | 신규 문제 생성 |
| 유사도 검증 | Python + FastAPI + 자체 임베딩 | 기존 문제와 유사도 판정(pgvector) |
| 문제 검증 | Python + FastAPI + Docker | 풀이 가능성·정답·테스트케이스 검증 |
...
```
사용자 제출 → Backend API → Kafka → Judge Worker(Go)
           → Docker Sandbox 실행 → 결과 판정 → DB 저장 → 프론트 표시
```
...
전체를 한 번에 만들지 않고 M1~M5로 세로 슬라이스 구현([TODO](../TODO.md)). POC는 Frontend 단독(mock 데이터).
